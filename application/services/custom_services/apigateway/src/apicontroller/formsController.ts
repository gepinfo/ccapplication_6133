...
export class formsController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/forms/:id', this.GpDelete);
this.router.get('/forms/get/search', this.GpSearch);
this.router.put('/forms/get/update', this.GpSearchForUpdate);
this.router.put('/forms', this.GpUpdate);
this.router.get('/forms/:id', this.GpGetEntityById);
this.router.get('/forms', this.GpGetAllValues);
this.router.post('/forms', this.GpCreate);
this.router.get('/forms/userid/created_by', this.GpGetNounCreatedBy);
        //#@gepdelimeterone@#
        //#@ssofacebookapiroute@#
        //#@ssogithubapiroute@#
        //#@gepbankingapiroute@#
...
public GpDelete(req: Request, res: Response) {
        ...
        new ApiAdapter().delete(Constant.FORMSERVICESURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.FORMSERVICESURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdapter().put(Constant.FORMSERVICESURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.FORMSERVICESURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
